...
async def _build_data_block(text: str, history: list[dict[str, Any]],
    ...
    for it in items:
        ...
        if not results:
            lines.append(f"— По запросу «{raw}»: в каталоге совпадений НЕТ (ни точных, ни близких). "
                         f"ЗАПРЕЩЕНО называть конкретное название/цену/наличие — таких товаров "
                         f"в ДАННЫХ просто нет, любое название или цифра тут будет выдумкой. "
                         f"Скажи честно, что такого в каталоге не нашлось, и предложи мягко: "
                         f"либо изготовление/заказ под размер (если это похоже на пиломатериал), "
                         f"либо зафиксировать запрос для менеджера — он проверит наличие и цену.")
            continue
        best = results[0]
