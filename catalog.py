...
# «бруса» — другой случай: это НЕ атрибут, различающий товары (как сухая/мокрая), а
# половина категорийного словосочетания «имитация бруса». В каталоге реальные позиции
# называются по профилю без слова «бруса» («Имитация Карельский профиль», «Имитация
# Скандинавский профиль») — если клиент назвал только общее «имитация бруса» (не назвал
# профиль), строгий AND-поиск требовал буквальное «бруса» в названии и находил 0, хотя
# сотни штук в наличии под именем без «бруса» (вопрос Артёма 17.08). «имитация» одна
# уже однозначно ведёт только в эту товарную линейку (проверено — других категорий с
# этим словом в каталоге нет), так что дропать «бруса» безопасно.
_GENERIC_MODIFIERS = {"обрезная", "обрезной", "необрезная", "бруса"}
